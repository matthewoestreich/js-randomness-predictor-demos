var app = (function () {
  ...
  function derived(fn) {
    ...
    var parent_derived = active_reaction !== null && (active_reaction.f & DERIVED) !== 0 ? /** @type {Derived} */ (active_reaction) : null;
    ...
    try {
      ...
      if (new_deps !== null) {
        ...
        if (
          ...
          ((flags & DERIVED) !== 0 && /** @type {import('#client').Derived} */ (reaction).reactions !== null)
        ) {
  ...
  var hasRequiredIndexBlXFM1MB;

  function requireIndexBlXFM1MB() {
    if (hasRequiredIndexBlXFM1MB) return indexBlXFM1MB;
    hasRequiredIndexBlXFM1MB = 1;
    var lowLevel = {};
    ...
    function requireLowLevel() {
      ...
      (function (exports2) {
        ...
        __exportStar(requireTypes___GENERATED__(), exports2);
        __exportStar(requireWrapper___GENERATED__(), exports2);
      })(lowLevel);
      return lowLevel;
    }
    indexBlXFM1MB.requireLowLevel = requireLowLevel;
    return indexBlXFM1MB;
  }

  var hasRequiredIndexChjXNfx0;

  function requireIndexChjXNfx0() {
    if (hasRequiredIndexChjXNfx0) return indexChjXNfx0;
    hasRequiredIndexChjXNfx0 = 1;
    const index$1 = requireIndexBlXFM1MB();
    ...
    var lowLevelExports = index$1.requireLowLevel();
    ...
    indexChjXNfx0.index = index;
    return indexChjXNfx0;
  }

  var indexCiaZbBfb = {};

  var hasRequiredIndexCiaZbBfb;

  function requireIndexCiaZbBfb() {
    if (hasRequiredIndexCiaZbBfb) return indexCiaZbBfb;
    hasRequiredIndexCiaZbBfb = 1;
    const index$1 = requireIndexBlXFM1MB();
    function _mergeNamespaces(n, m) {
      for (var i = 0; i < m.length; i++) {
        const e = m[i];
    ...
    function requireHighLevel$1() {
      ...
      const low_level_1 = index$1.requireLowLevel();
      const types_1 = requireTypes();
            ...
            updateValue(decl, a) {
              ...
              if (isFuncDecl(decl) && decl.arity() !== 0 && isFuncInterp(a)) {
                ...
                for (let i = 0; i < a.numEntries(); i++) {
                  const e = a.entry(i);
                  const n = e.numArgs();
                  const args = commonjsGlobal$1.Array(n).map((_, i2) => e.argValue(i2));
    ...
    indexCiaZbBfb.index = index;
    return indexCiaZbBfb;
  }
  ...
  function requireBrowser() {
    ...
    (function (exports) {
      ...
      class UnexpectedRuntimeError extends Error {
        constructor(message) {
          super(message);
      ...
      async function init() {
        ...
        const lowLevel = await (
          await Promise.resolve()
            .then(() => requireIndexChjXNfx0())
            .then((n) => n.index)
        ).init(initZ3Wrapper);
        const { createApi } = await Promise.resolve()
          .then(() => requireIndexCiaZbBfb())
          .then((n) => n.index);
        ...
        #getVersionSpecificMethods() {
          ...
          if (major <= 11) {
            return {
              ...
              constrainMantissa: (mantissa, symbolicState, solver, context) => {
      ...
      class BunRandomnessPredictor {
        #minimumSequenceLength = 6;
        #concreteState = [0n, 0n];
        #symbolicXor = (s) => XorShift128Plus.symbolic(s);
        #concreteXor = (s) => XorShift128Plus.concrete(s);
        // The mantissa bits (53 effective bits = 52 stored + 1 implicit) for doubles as defined in IEEE-754
        #IEEE754_MANTISSA_BITS_MASK = 0x1fffffffffffffn;
        // Map a 53-bit integer into the range [0, 1) as a double.
        #SCALING_FACTOR_53_BIT_INT = Math.pow(2, 53);
        constructor(sequence) {
          if (sequence && sequence.length < this.#minimumSequenceLength) {
            throw new InsufficientSequenceLengthError(`sequence length must be >= 6 : got ${sequence.length}`);
          }
          if (!sequence) {
      ...
      loader();
      const node = (sequence) => new NodeRandomnessPredictor(sequence);
      const firefox = (sequence) => new FirefoxRandomnessPredictor(sequence);
      const chrome = (sequence) => new ChromeRandomnessPredictor(sequence);
      const safari = (sequence) => new SafariRandomnessPredictor(sequence);
      const bun = (sequence) => new BunRandomnessPredictor(sequence);
      const deno = (sequence) => new DenoRandomnessPredictor(sequence);
      exports.BunRandomnessPredictor = BunRandomnessPredictor;
      exports.ChromeRandomnessPredictor = ChromeRandomnessPredictor;
      exports.DenoRandomnessPredictor = DenoRandomnessPredictor;
      exports.FirefoxRandomnessPredictor = FirefoxRandomnessPredictor;
      exports.NodeRandomnessPredictor = NodeRandomnessPredictor;
      exports.SafariRandomnessPredictor = SafariRandomnessPredictor;
      exports.bun = bun;
  ...
  var JSRandomnessPredictor = /*@__PURE__*/ getDefaultExportFromCjs(browserExports);
  ...
  var root_1 = add_locations(from_html(`<h1>Unsupported Browser! Please use Firefox, Chrome, or Safari</h1>`), App[FILENAME], [[94, 4]]);
  var root_4 = add_locations(
    from_html(
      `<tr><td class="table-data-cell svelte-1n46o8q"> </td><td class="table-data-cell svelte-1n46o8q"> </td><td class="table-data-cell svelte-1n46o8q"> </td><td class="table-data-cell svelte-1n46o8q"> </td></tr>`,
    ),
    App[FILENAME],
    [
      [
  ...
  var root_3 = add_locations(
    from_html(
      `<div class="table-container svelte-1n46o8q"><table style="border-collapse: separate; border-spacing: 0"><thead><tr><th class="table-header-cell svelte-1n46o8q"></th><th class="table-header-cell svelte-1n46o8q">Prediction</th><th class="table-header-cell svelte-1n46o8q">Math.random()</th><th class="table-header-cell svelte-1n46o8q">Correct?</th></tr></thead><tbody></tbody></table></div>`,
    ),
    App[FILENAME],
    [
      [
  ...
  var root_2 = add_locations(
    from_html(
      `<div><h3>You can either call <code>Math.random()</code> <code>Math.random()</code></h3> <button style="margin-right: 5px" class="svelte-1n46o8q">Make Prediction</button> <button class="svelte-1n46o8q">Call Math.random()</button> <br/> <p class="svelte-1n46o8q"> </p> <br/> <!></div>`,
    ),
    App[FILENAME],
    [
      [
  ...
  var root = add_locations(
    from_html(
      `<main><h1>Svelte + Rollup</h1> <p style="margin-bottom: 0" class="svelte-1n46o8q"><small class="svelte-1n46o8q"> </small></p> <p style="margin-top: 0" class="svelte-1n46o8q"><small class="svelte-1n46o8q"> </small></p> <h3><a href="https://github.com/matthewoestreich/js-randomness-predictor-demos/tree/main/vue/vite">Source Code</a></h3> <!></main>`,
    ),
    App[FILENAME],
    [
      [
  ...
  function App($$anchor, $$props) {
    ...
    const tableRowRefs = mutable_source([]);
    let tableContainerRef = mutable_source();

    // A 'prediction' has the following shape { prediction: number, random: number, correct: boolean }
    let predictions = mutable_source([]);

    let predictionIndex = 0;
    let randomIndex = 0;
    let status = mutable_source("");
    let sequence = mutable_source(null);
    let predictor = mutable_source(null);

    if (isSupported) {
      set(sequence, callMathRandomNTimes(strict_equals(browser, "safari") ? 6 : 4));
      set(predictor, JSRandomnessPredictor[browser](get(sequence)));
      console.log(...log_if_contains_state("log", get(predictor)));
    }

    async function scrollToRow(index) {
      (await track_reactivity_loss(tick()))();

      const container = get(tableContainerRef);
      const row = get(tableRowRefs)[index];
    ...
    async function handlePrediction() {
      set(
        status,
        strict_equals(browser, "firefox") || strict_equals(browser, "safari") ? "Working... (firefox|safari may take a bit longer)" : "Working...",
      );

      const prediction = (await track_reactivity_loss(get(predictor).predictNext()))();

      if (!get(predictions).length || !get(predictions)[predictionIndex]) {
        get(predictions).push({ prediction, random: null, correct: null });
      } else {
        mutate(predictions, (get(predictions)[predictionIndex].prediction = prediction));

        if (get(predictions)[predictionIndex].random) {
          mutate(predictions, (get(predictions)[predictionIndex].correct = strict_equals(get(predictions)[predictionIndex].random, prediction)));
        }
      }

      set(predictions, [...get(predictions)]);
      (await track_reactivity_loss(scrollToRow(predictionIndex)))();
    ...
    async function handleMathRandom(n = null) {
      ...
      if (!get(predictions).length || !get(predictions)[randomIndex]) {
        get(predictions).push({ prediction: null, random, correct: null });
      } else {
        mutate(predictions, (get(predictions)[randomIndex].random = random));

        if (get(predictions)[randomIndex].prediction) {
          mutate(predictions, (get(predictions)[randomIndex].correct = strict_equals(get(predictions)[randomIndex].prediction, random)));
        }
      }

      set(predictions, [...get(predictions)]);
      (await track_reactivity_loss(scrollToRow(randomIndex)))();
    ...
    var main = root();
    var p = sibling(child(main), 2);
    var small = child(p);
    var text = child(small);

    var p_1 = sibling(p, 2);
    var small_1 = child(p_1);
    var text_1 = child(small_1);

    var node = sibling(p_1, 4);

    {
      ...
      var alternate = ($$anchor) => {
        var div = root_2();
        var h3 = child(div);
        var text_2 = sibling(child(h3), 2);

        text_2.nodeValue = ' by clicking "Call Math.random()" or by opening your browser console and manually calling  ';

        var button = sibling(h3, 2);
        var button_1 = sibling(button, 2);
        var p_2 = sibling(button_1, 4);
        var text_3 = child(p_2);

        var node_1 = sibling(p_2, 4);

        {
          var consequent_1 = ($$anchor) => {
              ...
              () =>
                each(
                  ...
                  ($$anchor, prediction, index) => {
                    ...
                    bind_this(
                      tr,
                      ($$value, index) => mutate(tableRowRefs, (get(tableRowRefs)[index] = $$value)),
                      (index) => get(tableRowRefs)?.[index],
                      () => [get(index)],
                    );
                    ...
                    template_effect(
                      ($0) => {
                        ...
                        set_text(text_5, get(prediction).prediction ?? "");
                        set_text(text_6, get(prediction).random ?? "");
                        set_text(text_7, $0);
                      },
                      [() => (equals(get(prediction).correct, null) ? "" : get(prediction).correct.toString())],
                    );
            ...
            bind_this(
              div_1,
              ($$value) => set(tableContainerRef, $$value),
          ...
          add_svelte_meta(
            () =>
              if_block(node_1, ($$render) => {
                if (get(predictions).length > 0) $$render(consequent_1);
              }),
            "if",
            App,
            106,
        ...
        template_effect(() => {
          button.disabled = strict_equals(get(predictor), null) || strict_equals(get(status), "", false);
          button_1.disabled = strict_equals(get(predictor), null);
          set_text(text_3, strict_equals(get(status), "", false) ? "STATUS: " + get(status) : "");
        });

        event("click", button, () => handlePrediction());
        event("click", button_1, () => handleMathRandom());
        append($$anchor, div);
      };
      ...
      add_svelte_meta(
        () =>
          if_block(node, ($$render) => {
            if (!isSupported) $$render(consequent);
    ...
    template_effect(() => {
      set_text(text, `Browser: ${browser ?? ""}`);
      set_text(text_1, `Sequence: ${get(sequence) ?? ""}`);
    });

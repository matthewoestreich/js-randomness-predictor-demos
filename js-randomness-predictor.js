(function(global2, factory) {
  ...
  async function init() {
    ...
    const initZ32 = window.initZ3;
    const initZ3Wrapper = async () => await initZ32(window.Module);
    const lowLevel2 = await (await Promise.resolve().then(() => index$1)).init(initZ3Wrapper);
    const { createApi } = await Promise.resolve().then(() => index);
    const output = { ...lowLevel2, ...createApi(lowLevel2.Z3) };
    CACHED_Z3API = output;
    return output;
  }
  ...
  const firefox = JSRandomnessPredictor.firefox;
  const chrome = JSRandomnessPredictor.chrome;
  const safari = JSRandomnessPredictor.safari;
  (async () => {
    try {
  ...
  function requireWrapper___GENERATED__() {
    ...
    wrapper___GENERATED__.init = init2;
  ...
  var lowLevelExports = requireLowLevel();
  ...
  var highLevelExports = requireHighLevel();
